...
export function startCronJob() {
  ...
  checkDeliveryStatus().catch(error => {
    console.error(`[${new Date().toISOString()}] Error in initial cron job run:`, error);
  });
  ...
  cronJobInterval = setInterval(() => {
    ...
    checkDeliveryStatus().catch(error => {
      console.error(`[${new Date().toISOString()}] Error in scheduled cron job run:`, error);
    });
  }, CRON_INTERVAL);
  ...
  try {
    ...
    if (currentTime - lastRunTime >= CRON_INTERVAL) {
      ...
      return res.status(200).json({ 
        message: 'Delivery status check completed successfully',
